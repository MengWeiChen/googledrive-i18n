...
const scanSheet = async (auth, sheetId) => {
  ...
  const targetGridName = gridNameList.filter(
    gridNmae => gridNmae.indexOf('_') !== 0
  )
...
const main = async (authPath, outputPath, rootFolderId, sheetId) => {
  const auth = await getAuth(authPath)
  let result
  if (!sheetId) {
    result = await scanFolder(auth, rootFolderId)
  } else {
    result = await scanSheet(auth, sheetId)
  }
